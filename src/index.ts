...
const smartUpdateAll = async () => {
  ...
  const testPkgs = async (suspects: string[], latest = true): Promise<boolean> => {
  ...
  while (withStatus(undefined).length || withStatus('yellow').length) {
    ...
    if (remainingSuspects.length === 1) {
      statuses.set(
        ...
        (await testPkgs(remainingSuspects, false)) ? 'magenta' : 'red'
      );
    }
    printStatuses();
  }
...
//   await saveBackup();
//   latest ? await smartUpdateAll() : await smartUpdateCompatible();
//   await deleteBackup();
// };
